...
void Axis::setDPOS(Distance d) {
  ...
  //         std::cout << "Position not reached, timeout reached. (4) " << d <<
  //         std::endl; error = true; break;
  //     }
...
void Axis::sendSettings() {
  char cmd[64];
  std::cout << "Axis " << letter_
            << ": sending settings count = " << settings_.size() << std::endl;
...
int Axis::applySettingsMultipliers_(std::string tag, int value) {
  if (tag == "MAMP" || tag == "OSFA" || tag == "OFSB" || tag == "AMPL" ||
      tag == "MAM2")
  ...
  else if (tag == "LLIM" || tag == "RLIM" || tag == "HLIM" || tag == "ZON1" ||
           tag == "ZON2")
...
void Axis::setSetting(std::string tag, std::string value,
                      bool fromSettingsFile) {
...
void Axis::receiveData(std::string tag, int value) {
  ...
  // std::cout << "Received: " << letter_ << " " << tag << " = " << value <<
  // std::endl;
...
void Axis::calibrate() {
  ...
  while (isSearchingOptimalFrequency()) {
    ...
    int freq_ratio =
        ((FRQ - LFRQ) * 100) / (HFRQ - LFRQ); // Calculate % finished
    if (freq_ratio % 10 == 0) {
