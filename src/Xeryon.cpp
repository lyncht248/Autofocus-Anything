...
void Xeryon::start() {
  if (axis_list_.size() == 0) {
    std::cout << "Cannot start the system without stages. The stages don't "
                 "have to be connnected, only initialized in the software."
              << std::endl;
    return;
  }

#if defined(_WIN32) || defined(_WIN64)
  port_handle_ = CreateFileA(port_, GENERIC_READ | GENERIC_WRITE, 0, 0,
  ...
  // Restore non-blocking mode as originally tuned
...
#endif
#if defined(__linux__) || defined(__APPLE__)
  // Robust write: handle partial writes and transient errors
  ssize_t total = 0;
  while (total < n) {
    ssize_t rc = write(port_fd_, cmd + total, n - total);
    if (rc > 0) {
      total += rc;
      continue;
    }
    if (rc == -1 &&
      ...
      (void)poll(&p, 1, 50); // wait briefly for port to become writable
      continue;
    }
    // Hard failure
...
void Xeryon::readSettings() {
  // Try multiple candidate locations so running from different CWDs works
  std::vector<std::string> candidates = {
      "settings_default.txt", "../settings_default.txt",
      "config/settings_default.txt", "../config/settings_default.txt"};
  ...
  if (!file.is_open()) {
    std::cout << "Xeryon::readSettings: failed to open settings_default.txt "
                 "from candidate paths"
              << std::endl;
    return;
  }
  std::cout << "Xeryon::readSettings: using settings file: " << usedPath
            << std::endl;
  std::string line;
  std::size_t pos;
  int parsedCount = 0;
  ...
  file.close();
  std::cout << "Xeryon::readSettings: parsed settings entries: " << parsedCount
            << std::endl;
