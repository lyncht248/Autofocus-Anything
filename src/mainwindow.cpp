...
struct MainWindow::Private {
  Gtk::Label gainLabel, exposeLabel, gammaLabel, frameRateLabel, thresLabel,
      scaleLabel, recordingSizeLabel, bestFocusLabel, homePositionLabel;
  // Gtk::Label waitLabel; //This was used to time-out the stabilizer.cc code,
  ...
  Gtk::Label autofocusTitle, stabilizationTitle, playbackTitle,
      cameraSettingsTitle, fileTitle;
  Gtk::Separator verticalSeparator1, verticalSeparator2, verticalSeparator3,
      verticalSeparator4;
...
MainWindow::Private::Private()
    : gainLabel("Gain: "), exposeLabel("Expose (us): "), gammaLabel("Gamma: "),
      frameRateLabel("Frame rate: "),
      // thresLabel("Threshold: "),
      // scaleLabel("Scale: "),
      // waitLabel("Optimize: "),
      recordingSizeLabel("Rec. Size: "), bestFocusLabel("Best-Focal Plane"),
      autofocusTitle("Autofocus"), stabilizationTitle("XY Stabilization"),
      playbackTitle("Playback Controls"),
      cameraSettingsTitle("Camera Settings"), fileTitle("File Load and Save"),
      homePositionLabel("Home Position (mm): "), controlFrame(), controlGrid(),
      verticalSeparator1(), verticalSeparator2(), verticalSeparator3(),
      verticalSeparator4(),
      rootBox(), space4{Gtk::Label("    "), Gtk::Label("    "),
                        Gtk::Label("    "), Gtk::Label("    "),
                        Gtk::Label("    "), Gtk::Label("    "),
                        Gtk::Label("    "), Gtk::Label("    "),
                        Gtk::Label("    "), Gtk::Label("    "),
                        ...
                        Gtk::Label("    "), Gtk::Label("    "),
                        Gtk::Label("    "), Gtk::Label("    ")},
      // verticalLine{Gtk::Label(" | "), Gtk::Label("  |  "), Gtk::Label("  |
      // "), Gtk::Label("  |  "), Gtk::Label("  |  "), Gtk::Label("  |  "),
      // Gtk::Label("  |  "), Gtk::Label("  |  ")},
      mediaBox(Gtk::Orientation::ORIENTATION_HORIZONTAL), fileChooserBox(),
      displayBox(), sharpnessLabel("Focus Profile") {
...
ScaleWidget::ScaleWidget(double lower, double upper, double inc, double def,
                         int spinButtonWidth, int scaleWidth, bool stepSnap)
    : Gtk::Bin(), sigChanged(), scale(), spinButton(), grid(),
      stepSnap(stepSnap) {
  if (stepSnap) {
  ...
  scaleConnection = scale.signal_value_changed().connect(
      sigc::mem_fun(*this, &ScaleWidget::scaleChanged));
  if (scaleWidth)
  ...
  spinButtonConnection = spinButton.signal_value_changed().connect(
      sigc::mem_fun(*this, &ScaleWidget::spinButtonChanged));
  if (spinButtonWidth)
...
ScaleWidget::SignalChanged ScaleWidget::signalChanged() { return sigChanged; }
...
double ScaleWidget::getValue() const { return spinButton.get_value(); }

void ScaleWidget::setValue(double v) { spinButton.set_value(v); }
...
void ScaleWidget::setUpperLimit(double value) {
  ...
  if (stepSnap) {
    scale.set_range(min, (a->get_step_increment() * min - min + value) /
                             a->get_step_increment());
    scale.set_value(min +
                    (spinButton.get_value() - min) * a->get_step_increment());
  } else {
...
bool MainWindow::_on_state_event(GdkEventWindowState *window_state_event) {
  if (!(window_state_event->new_window_state &
        (GdkWindowState::GDK_WINDOW_STATE_WITHDRAWN |
         GdkWindowState::GDK_WINDOW_STATE_ICONIFIED))) {
  ...
  if (window_state_event->new_window_state &
      GdkWindowState::GDK_WINDOW_STATE_FOCUSED) {
    SDLWindow::raise(childwin);
  } else {
...
MainWindow::MainWindow()
    : Gtk::Window(), priv(new Private()),
      ...
      bestFocusScale(190, 450, 20, 240, 4, 100),
      ...
      stabiliseToggle("XY Stab."), showMapToggle("Show Map"),
      findFocusToggle("Find Focus"), holdFocusToggle("Hold Focus"),
      threedStabToggle("3D Stab."), twodStabToggle("2D Stab."), fpsLabel(""),
      loadSaveLabel(""), errorLabel(""),
      // display(*this),
      drawFrame(nullptr), newDrawFrame(false), countFrames(),
      // makeMapActive(),
      stabiliseActive(), showMapActive(), holdFocusActive(), threedStabActive(),
      twodStabActive(), hasBuffer(), liveView(), loading(), saving(),
      playingBuffer(), seeking(), recording(), pausedRecording(), trackingFPS(),
      sigFrameDrawn(), sigFeatureUpdated(),
      // sigThresholdChanged(),
      // sigScaleChanged(),
      sigBestFocusChanged(), gainScaleConnection(), exposeScaleConnection(),
      gammaScaleConnection(),
      // frameRateScaleConnection(),
      frameSliderConnection(), stateChangeConnection(),
      homePositionScaleConnection(), pGainScaleConnection(), getDepthsToggle(),
      viewDepthsToggle(), viewDepthsActive(), getDepthsActive(),
      settings("") // Use default settings path from settings.cpp
{
  ...
  stateChangeConnection = signal_window_state_event().connect(
      sigc::mem_fun(*this, &MainWindow::_on_state_event));
  // gainScale.setScaleSizeRequest(150, 0);
  // gainScale.setSpinButtonWidth(7);
  gainScaleConnection = gainScale.signalChanged().connect(
      sigc::mem_fun(*this, &MainWindow::onGainScaleChange));

  // exposeScale.setScaleSizeRequest(150, 0);
  // exposeScale.setSpinButtonWidth(7);
  exposeScaleConnection = exposeScale.signalChanged().connect(
      sigc::mem_fun(*this, &MainWindow::onExposeScaleChange));

  // gammaScale.setScaleSizeRequest(150, 0);
  // gammaScale.setSpinButtonWidth(7);
  gammaScale.setSpinButtonPrec(2);
  gammaScaleConnection = gammaScale.signalChanged().connect(
      sigc::mem_fun(*this, &MainWindow::onGammaScaleChange));

  // frameRateScaleConnection =
  // frameRateScale.signalChanged().connect(sigc::mem_fun(*this,
  // &MainWindow::onFrameRateChange) );

  recenterButton.set_sensitive(false);

  // Tooltip text for the buttons
  // makeMapToggle.set_tooltip_text("Make a vessel map of the current recording
  // using given threshold and scale values");
  stabiliseToggle.set_tooltip_text(
      "Using vessel map, XY-stabilise the current recording");
  // showMapToggle.set_tooltip_text("Show the loaded vessel map");
  findFocusToggle.set_tooltip_text(
      "Finds focal plane with highest sharpness and then holds it in focus");
  holdFocusToggle.set_tooltip_text(
      "Holds current focal plane in-focus (even if not the highest sharpness)");
  threedStabToggle.set_tooltip_text(
      "Shortcut for live angiograms which uses 'Hold Focus' and 'XY-Stab'");
  ...
  recordButton.signal_clicked().connect(
      sigc::mem_fun(*this, &MainWindow::onRecordClicked));
  ...
  recordButton.get_style_context()->add_provider(
  ...
  backToStartButton.signal_clicked().connect(
      sigc::mem_fun(*this, &MainWindow::onBackButtonClicked));
  ...
  backToStartButton.get_style_context()->add_provider(
      cssProvider2, GTK_STYLE_PROVIDER_PRIORITY_USER);
  ...
  pauseButton.signal_clicked().connect(
      sigc::mem_fun(*this, &MainWindow::onPauseClicked));
  ...
  pauseButton.get_style_context()->add_provider(
      cssProvider3, GTK_STYLE_PROVIDER_PRIORITY_USER);
  ...
  playButton.get_style_context()->add_provider(
      cssProvider4, GTK_STYLE_PROVIDER_PRIORITY_USER);
  ...
  liveToggle.signal_toggled().connect(
      sigc::mem_fun(*this, &MainWindow::onLiveToggled));
  ...
  frameSliderConnection = frameSlider.signalChanged().connect(
      sigc::mem_fun(*this, &MainWindow::onFrameSliderChange));

  fileLoadButton.set_tooltip_text("Load frames from this location");
  fileLoadButton.signal_clicked().connect(
      sigc::mem_fun(*this, &MainWindow::onLoadButtonClicked));
  ...
  fileSaveButton.signal_clicked().connect(
      sigc::mem_fun(*this, &MainWindow::onSaveButtonClicked));

  settingsButton.set_image_from_icon_name("settings");
  settingsButton.set_tooltip_text("Settings");
  settingsButton.set_hexpand(false);
  settingsButton.set_valign(Gtk::Align::ALIGN_FILL);
  settingsButton.set_halign(Gtk::Align::ALIGN_END);

  // Style the settings button with white icon
  auto cssProviderSettings = Gtk::CssProvider::create();
  Glib::ustring cssDataSettings =
      R"(
		#settingsButton {
			color: #ffffff;  
			-gtk-icon-style: symbolic;
  ...
  cssProviderSettings->load_from_data(cssDataSettings);
  settingsButton.get_style_context()->add_provider(
      cssProviderSettings, GTK_STYLE_PROVIDER_PRIORITY_USER);
  settingsButton.set_name("settingsButton");
  ...
  settingsButton.signal_clicked().connect([this]() {
    ...
    returnPositionSlider.set_range(settings.getMinPosition(),
                                   settings.getMaxPosition());
  ...
  // thresScale.signalChanged().connect(sigc::mem_fun(*this,
  // &MainWindow::onThresScaleChange) );

  // scaleScale.setScaleSizeRequest(100, 0);
  // scaleScale.setSpinButtonWidth(4);
  // scaleScale.setSpinButtonPrec(2);
  // scaleScale.signalChanged().connect(sigc::mem_fun(*this,
  // &MainWindow::onScaleScaleChange) );
  recordingSizeScale.set_tooltip_text(
      "Set the maximum number of frames for a single recording");
  recordingSizeScale.signalChanged().connect(
      sigc::mem_fun(*this, &MainWindow::onRecordingSizeScaleChange));
  ...
  bestFocusScale.signalChanged().connect(
      sigc::mem_fun(*this, &MainWindow::onBestFocusScaleChange));
  //
  ...
  // display.signal_draw().connect(sigc::mem_fun(*this,
  // &MainWindow::renderDisplay) );
  ...
  // makeMapActive.signalToggled().connect(sigc::mem_fun(*this,
  // &MainWindow::whenMakeMapToggled) );

  stabiliseActive.toggleOnSignal(stabiliseToggle.signal_toggled());
  stabiliseActive.signalToggled().connect(
      sigc::mem_fun(*this, &MainWindow::whenStabiliseToggled));

  // showMapActive.toggleOnSignal(showMapToggle.signal_toggled() );
  // showMapActive.signalToggled().connect(sigc::mem_fun(*this,
  // &MainWindow::whenShowMapToggled) );

  findFocusToggle.signal_toggled().connect(
      sigc::mem_fun(*this, &MainWindow::onFindFocusToggled));

  resetButton.signal_clicked().connect(
      sigc::mem_fun(*this, &MainWindow::onResetClicked));
  ...
  getDepthsToggle.set_tooltip_text(
      "Generate depth map by scanning focus positions");
  ...
  holdFocusActive.signalToggled().connect(
      sigc::mem_fun(*this, &MainWindow::whenHoldFocusToggled));

  getDepthsActive.toggleOnSignal(getDepthsToggle.signal_toggled());
  getDepthsActive.signalToggled().connect(
      sigc::mem_fun(*this, &MainWindow::whenGetDepthsToggled));

  viewDepthsActive.toggleOnSignal(viewDepthsToggle.signal_toggled());
  viewDepthsActive.signalToggled().connect(
      sigc::mem_fun(*this, &MainWindow::whenViewDepthsToggled));

  threedStabActive.toggleOnSignal(threedStabToggle.signal_toggled());
  threedStabActive.signalToggled().connect(
      sigc::mem_fun(*this, &MainWindow::when3DStabToggled));

  // twodStabActive.toggleOnSignal(twodStabToggle.signal_toggled() );
  // twodStabActive.signalToggled().connect(sigc::mem_fun(*this,
  // &MainWindow::when2DStabToggled) );

  hasBuffer.signalTrue().connect(
      sigc::mem_fun(*this, &MainWindow::bufferFilled));
  hasBuffer.signalFalse().connect(
      sigc::mem_fun(*this, &MainWindow::bufferEmptied));

  // Viewing means we're not in liveView, and we either have a buffer or we're
  // loading another buffer
  Condition &viewing = !liveView && (hasBuffer || loading);

  viewing.signalFalse().connect(sigc::mem_fun(*this, &MainWindow::viewingLive));
  viewing.signalTrue().connect(
      sigc::mem_fun(*this, &MainWindow::viewingBuffer));

  loading.signalToggled().connect(
      sigc::mem_fun(*this, &MainWindow::whenLoadingToggled));
  saving.signalToggled().connect(
      sigc::mem_fun(*this, &MainWindow::whenSavingToggled));

  recording.signalToggled().connect(
      sigc::mem_fun(*this, &MainWindow::whenRecordingToggled));
  pausedRecording.signalToggled().connect(
      sigc::mem_fun(*this, &MainWindow::whenPausedRecordingToggled));

  trackingFPS.signalToggled().connect(
      sigc::mem_fun(*this, &MainWindow::whenTrackingFPSToggled));
  trackingFPS.setValue(true);

  playingBuffer.signalToggled().connect(
      sigc::mem_fun(*this, &MainWindow::whenPlayingBufferToggled));
  ...
  fileLoadButton.set_size_request(
      125, -1); // Width: 120px, Height: keep default (-1)
  fileSaveButton.set_size_request(
      125, -1); // Width: 120px, Height: keep default (-1)

  enterButton.signal_clicked().connect(sigEnterClicked.make_slot());
  Gtk::Box *entryButtonBox =
      Gtk::manage(new Gtk::Box(Gtk::Orientation::ORIENTATION_HORIZONTAL, 0));
  entryButtonBox->pack_start(frameRateEntry,
                             Gtk::PackOptions::PACK_EXPAND_WIDGET);
  entryButtonBox->pack_start(enterButton, Gtk::PackOptions::PACK_SHRINK);
  enterButton.set_size_request(
      74, -1); // 100 pixels in width, -1 means natural height
  ...
  // Setup warning label
  outOfBoundsWarningLabel.set_markup(
      "<span foreground='orange'>\u26A0 Lens Out-of-Bounds\n    Autofocus May "
      "Fail</span>");
  ...
  // priv->controlGrid.attach(homePositionScale, 3, 2);	// Uncomment to give
  // user control of home position
  //  Add warning label below best focus scale in the grid when lens is out of
  //  bounds
  // priv->controlGrid.attach(pGainScale, 3, 2); // Unncomment to give user
  // control of P gain
  ...
  priv->controlGrid.attach(
      settingsButton, 24,
      0); // Positioned at 24, 0 to overlap with sharpnessLabel
...
void MainWindow::updateCameraValues(double gain, double expose, double gamma) {
  ...
  if (bMainWindowLogFlag) {
    logger->info("[MainWindow::updateCameraValues] gainScale, exposeScale, "
                 "gammaScale updated");
  }
}
...
void MainWindow::getDisplayDimensions(double &w, double &h) const {
  ...
  if (bMainWindowLogFlag) {
    logger->info("[MainWindow::getDisplayDimensions] GTK display dimensions of "
                 "800,600 returned");
  }
}
...
void MainWindow::setHasBuffer(bool val) { hasBuffer.setValue(val); }

Condition &MainWindow::getLoading() { return loading; }

Condition &MainWindow::getSaving() { return saving; }
...
void MainWindow::setLoading(bool val) { loading.setValue(val); }

void MainWindow::setSaving(bool val) { saving.setValue(val); }
...
void MainWindow::setSeeking(bool val) { seeking.setValue(val); }

void MainWindow::setRecording(bool val) { recording.setValue(val); }
...
void MainWindow::set3DStab(bool val) { threedStabToggle.set_active(val); }

void MainWindow::setHoldFocus(bool val) { holdFocusToggle.set_active(val); }

void MainWindow::setTrackingFPS(bool val) { trackingFPS.setValue(val); }
...
Condition &MainWindow::getStabiliseActive() { return stabiliseActive; }
...
Condition &MainWindow::getHoldFocusActive() { return holdFocusActive; }

Condition &MainWindow::get3DStabActive() { return threedStabActive; }

Condition &MainWindow::get2DStabActive() { return twodStabActive; }

Condition &MainWindow::getHasBuffer() { return hasBuffer; }

Condition &MainWindow::getLiveView() { return liveView; }

Condition &MainWindow::getPlayingBuffer() { return playingBuffer; }

Condition &MainWindow::getSeeking() { return seeking; }

Condition &MainWindow::getRecording() { return recording; }

Condition &MainWindow::getPausedRecording() { return pausedRecording; }

int MainWindow::getFrameSliderValue() const { return frameSlider.getValue(); }

std::string MainWindow::getFileLocation() const { return current_file_path; }
...
void MainWindow::on_show() {
  ...
  if (bMainWindowLogFlag)
    logger->info("[MainWindow::on_show] SDL childwin moved to " +
                 std::to_string((dim.get_width() - 800) / 2) + "," +
                 std::to_string(y + get_height()));
}
...
bool MainWindow::renderDisplay(const ::Cairo::RefPtr<::Cairo::Context> &cr) {
  ...
  if (drawFrame)
  {
          double xscale = (double) (display.get_width() ) /
  drawFrame->get_width(); double yscale = (double) (display.get_height() ) /
  drawFrame->get_height(); if (xscale < 1.0 || yscale < 1.0)
          {
                  double arscale = drawFrame->get_width() >
  drawFrame->get_height() ? yscale : xscale; cr->scale(arscale, arscale);
          }
...
// 	    if(bMainWindowLogFlag)
// {logger->info("[MainWindow::whenMakeMapToggled] MakeMap toggled on");}
...
// 	    if(bMainWindowLogFlag)
// {logger->info("[MainWindow::whenMakeMapToggled] MakeMap toggled off");}
// 	}
// }
...
// 		if(bMainWindowLogFlag)
// logger->info("[MainWindow::whenShowMapToggled] ShowMap toggled on");
// 	}
// 	else
// 	{
// 		//GUI CHANGES WHEN "SHOW MAP" IS DISABLED GO HERE
// 		if(bMainWindowLogFlag)
// logger->info("[MainWindow::whenShowMapToggled] ShowMap toggled off");
// 	}
// }
...
void MainWindow::onFindFocusToggled() {
  if (findFocusToggle.get_active()) {
    ...
    if (bMainWindowLogFlag) {
      logger->info("[MainWindow::onFindFocusToggled] FindFocus toggled on, "
                   "bFindFocus set to 1");
    }
    ...
    Glib::signal_timeout().connect_once(
        [this]() {
          ...
          if (findFocusToggle.get_active()) { // Check if still active
            bHoldFocus = 1;
            if (bMainWindowLogFlag) {
              logger->info("[MainWindow::onFindFocusToggled] Transitioning to "
                           "hold focus mode");
            }
          }
        },
...
void MainWindow::whenHoldFocusToggled(bool holdingFocus) {
  if (holdingFocus) {
    // Make best focus scale active and set value to the desired location of
    // best-focus
    bestFocusScale.set_sensitive(true);
...
void MainWindow::onResetClicked() {
  ...
  if (bMainWindowLogFlag) {
    logger->info("[MainWindow::onResetClicked] Reset All clicked - performing "
                 "comprehensive reset");
  }
  ...
  getDepthsToggle.set_active(
      false); // This will trigger clearing via whenGetDepthsToggled
  ...
  if (bMainWindowLogFlag) {
    logger->info(
        "[MainWindow::onResetClicked] Reset All complete - all systems reset");
  }
}
...
void MainWindow::bufferEmptied() {
  if (!recording.getValue()) {
    ...
    if (bMainWindowLogFlag) {
      logger->info("[MainWindow::bufferEmptied] Recording buffer emptied... "
                   "although nothing is deleted??");
    }
  }
}
...
void MainWindow::whenRecordingToggled(bool recording) {
  if (recording) {
    ...
    trackingFPS.setValue(false); // Stop tracking FPS when recording. TODO: Add
                                 // second label to show FPS while recording.
...
void MainWindow::whenPausedRecordingToggled(bool paused) {
  if (paused) {
    ...
    fileSaveButton.set_sensitive(true);
    if (bMainWindowLogFlag) {
      logger->info("[MainWindow::whenPausedRecordingToggled] Paused recording "
                   "toggled on");
    }
  } else {
    recordButton.set_sensitive(false);
...
void MainWindow::whenTrackingFPSToggled(bool tracking) {
  if (tracking) {
    ...
    Glib::signal_timeout().connect(
        sigc::mem_fun(*this, &MainWindow::updateFPSCounter), 1000);
    if (bMainWindowLogFlag) {
      logger->info(
          "[MainWindow::whenTrackingFPSToggled] Tracking FPS toggled on");
    }
  }
}
...
void MainWindow::onSaveButtonClicked() {
  Gtk::FileChooserDialog dialog(
      "Enter a folder name",
      Gtk::FileChooserAction::FILE_CHOOSER_ACTION_CREATE_FOLDER);
  dialog.set_transient_for(*this);
  dialog.set_modal(true);
...
void MainWindow::onLiveToggled() { liveView.setValue(liveToggle.get_active()); }
...
void MainWindow::whenPlayingBufferToggled(bool playing) {
  if (playing) {
    ...
    if (bMainWindowLogFlag) {
      logger->info(
          "[MainWindow::whenPlayingBufferToggled] Playing buffer toggled on");
    }
  } else {
    playButton.set_sensitive(true);
    ...
    if (bMainWindowLogFlag) {
      logger->info(
          "[MainWindow::whenPlayingBufferToggled] Playing buffer toggled off");
    }
  }
}
...
MainWindow::SignalSettingsChanged MainWindow::signalSettingsChanged() {
  return sigSettingsChanged;
}
...
MainWindow::~MainWindow() {
  ...
  if (bMainWindowLogFlag) {
    logger->info(
        "[MainWindow::~MainWindow] destructor completed, priv deleted");
  }
}
...
void MainWindow::setStabiliseActive(bool val) {
  ...
  if (bMainWindowLogFlag) {
    logger->info("[MainWindow::setStabiliseActive] Stabilise active set to {}",
                 val);
  }
}

void MainWindow::setViewDepths(bool val) { viewDepthsToggle.set_active(val); }

Condition &MainWindow::getFindFocusActive() {
  // Note: We don't actually have a condition for this, so we'll use
  // holdFocusActive for now You might want to add a proper findFocusActive
  // condition if needed
...
void MainWindow::whenGetDepthsToggled(bool gettingDepths) {
  if (gettingDepths) {
    ...
    if (bMainWindowLogFlag) {
      logger->info("[MainWindow::whenGetDepthsToggled] Get Depths toggled on - "
                   "starting depth mapping");
    }
  } else {
    ...
    if (bMainWindowLogFlag) {
      logger->info("[MainWindow::whenGetDepthsToggled] Get Depths toggled off "
                   "- clearing depth mapping");
    }
  }
}
