...
SDLWin *SDLWindow::sdlwin_open() {
  // Fork a process and use shared memory because the SDL renderer doesn't like
  // running on any thread other than main, and this process already has GTK on
  // the main thread.
  //  This spawns sdlwinchild, which runs on a process from child_main()
  SDLWin *out =
      (struct SDLWin *)mmap(NULL, sizeof(struct SDLWin), PROT_READ | PROT_WRITE,
...
Response SDLWindow::renderFrameG8(SDLWin *win, const void *buf, size_t n,
                                  int xoff, int yoff) {
  lockcmd(win);
  if (buf)
...
unsigned long long SDLWindow::upload(SDLWin *win, const void *buf, size_t n,
                                     bool lock, int i, unsigned long long off) {
  if (lock)
...
Response SDLWindow::updateMap(SDLWin *win, const void *buf, int width,
...
void handleDoubleClick(SDLWin *win, int x, int y) {
  if (win && win->system) {
    ...
    std::cout << "[SDL Window] Double-click at (" << imageX << ", " << imageY
              << "), ROI updated" << std::endl;
  }
}

void updateROIDisplay(SDLWin *win) {
  if (win && win->system && win->system->getSensorFusion()) {
    // Get ROI info directly from sensor fusion to avoid mutex deadlock
    int centerX, centerY, width, height;
    win->system->getSensorFusion()->getCurrentROI(centerX, centerY, width,
                                                  height);
...
void drawROIOverlay(SDLWin *win, SDL_Renderer *renderer) {
  ...
  if (win->roiCenterX >= 0 && win->roiCenterY >= 0) {
    ...
    for (int i = 1; i < 3; i++) {
      SDL_Rect thickRect = {roiX - i, roiY - i, win->roiWidth + 2 * i,
                            win->roiHeight + 2 * i};
      SDL_RenderDrawRect(renderer, &thickRect);
    }

    // Draw center crosshair
    SDL_RenderDrawLine(renderer, win->roiCenterX - 5, win->roiCenterY,
                       win->roiCenterX + 5, win->roiCenterY);
    SDL_RenderDrawLine(renderer, win->roiCenterX, win->roiCenterY - 5,
                       win->roiCenterX, win->roiCenterY + 5);
  }
}

void SDLWindow::transferDepthMapData(
    SDLWin *win,
    const std::vector<std::vector<std::pair<double, double>>> &depthImage,
    int width, int height) {
  std::cout << "[Debug] transferDepthMapData() called with " << width << "x"
            << height << " depth image" << std::endl;
  ...
  if (width <= SDLWin::MAX_DEPTH_WIDTH && height <= SDLWin::MAX_DEPTH_HEIGHT) {
    ...
    if (hasValidData) {
      ...
      std::cout << "[Debug] Calculated dynamic focus range: " << minFocus
                << " - " << maxFocus << std::endl;
    } else {
      // Fallback to default values if no valid data
      win->focusMin = 190.0f;
      win->focusMax = 450.0f;
      std::cout << "[Debug] No valid focus data found, using default range: "
                << win->focusMin << " - " << win->focusMax << std::endl;
    }
    ...
    std::cout << "[SDL Window] Depth map dimensions too large: " << width << "x"
              << height << " (max: " << SDLWin::MAX_DEPTH_WIDTH << "x"
              << SDLWin::MAX_DEPTH_HEIGHT << ")" << std::endl;
