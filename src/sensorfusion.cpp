...
SensorFusion::SensorFusion(System &system)
  ...
  std::cout << "[Sensor Fusion] Ready. Double-click on video to set ROI and "
               "start sharpness analysis."
            << std::endl;
...
void SensorFusion::start() {
  ...
  m_running = true;
  m_monitorThread = std::thread(&SensorFusion::monitorThreadFunction, this);
  std::cout << "[Sensor Fusion] Started ROI sharpness monitoring thread (5Hz, "
...
void SensorFusion::stop() {
  ...
  std::cout << "[Sensor Fusion] Stopped ROI sharpness monitoring thread"
            << std::endl;
}

void SensorFusion::setROISize(int width, int height) {
  std::lock_guard<std::mutex> lock(m_roiMutex);
  m_roiWidth = width;
  m_roiHeight = height;
  std::cout << "[Sensor Fusion] ROI size set to " << width << "x" << height
            << std::endl;
}

void SensorFusion::setROICenter(int centerX, int centerY) {
  std::lock_guard<std::mutex> lock(m_roiMutex);
  m_roiCenterX = centerX;
  m_roiCenterY = centerY;
  m_useCustomCenter = true;
  std::cout << "[Sensor Fusion] ROI center set to (" << centerX << ", "
            << centerY << ")" << std::endl;
}

void SensorFusion::getCurrentROI(int &centerX, int &centerY, int &width,
                                 int &height) const {
  std::lock_guard<std::mutex> lock(m_roiMutex);
...
void SensorFusion::startROIFocusSearch() {
  if (m_focusSearchActive.load()) {
    std::cout << "[Sensor Fusion] Focus search already in progress"
              << std::endl;
    return;
  }

  if (!m_useCustomCenter.load()) {
    std::cout << "[Sensor Fusion] No ROI selected, cannot start focus search"
              << std::endl;
    return;
  }
  ...
  if (!testFrame || !testFrame->data() || testFrame->size().x <= 0 ||
      testFrame->size().y <= 0) {
    std::cout
        << "[Sensor Fusion] No valid frame available, cannot start focus search"
        << std::endl;
    return;
  }

  std::cout << "[Sensor Fusion] Starting ROI focus search..." << std::endl;
  m_focusSearchRequested = true;
}

void SensorFusion::startDepthMapping() {
  if (m_depthMappingActive.load()) {
    std::cout << "[Sensor Fusion] Depth mapping already in progress"
              << std::endl;
    return;
  }
  ...
  if (!m_running.load()) {
    start();
    std::cout << "[Sensor Fusion] Started monitoring thread for depth mapping"
              << std::endl;
  }

  std::cout << "[Sensor Fusion] Starting depth mapping..." << std::endl;
  m_depthMappingRequested = true;
}

void SensorFusion::monitorThreadFunction() {
  while (m_running.load()) {
    auto now = std::chrono::steady_clock::now();
    auto elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(
        now - m_lastProcessTime);
    ...
    if (m_focusSearchRequested.load() && !m_focusSearchActive.load()) {
      ...
      try {
        // Perform focus search in a separate thread to avoid blocking
        // monitoring
        std::thread searchThread(&SensorFusion::performFocusSearch, this);
        searchThread.detach();
      } catch (const std::exception &e) {
        std::cerr << "[Sensor Fusion] Error starting focus search thread: "
                  << e.what() << std::endl;
        m_focusSearchActive = false;
      }
    }
    ...
    if (m_depthMappingRequested.load() && !m_depthMappingActive.load()) {
      ...
      try {
        // Perform depth mapping in a separate thread to avoid blocking
        // monitoring
        std::thread depthThread(&SensorFusion::performDepthMapping, this);
        depthThread.detach();
      } catch (const std::exception &e) {
        std::cerr << "[Sensor Fusion] Error starting depth mapping thread: "
                  << e.what() << std::endl;
        m_depthMappingActive = false;
      }
    }
    ...
    if (elapsed >= m_processInterval) {
      ...
      if (m_useCustomCenter.load()) {
        ...
        if (currentFrame && currentFrame->data() &&
            currentFrame->size().x > 0 && currentFrame->size().y > 0) {
          try {
            // Convert VidFrame to OpenCV Mat
            cv::Mat frame(currentFrame->size().y, currentFrame->size().x,
                          CV_8UC1, (void *)currentFrame->data());
            ...
            if (!frame.empty()) {
              ...
              if (!roi.empty()) {
                ...
                // Print to console with timestamp and ACTUAL analyzed ROI
                // coordinates
                auto timestamp =
                ...
                // Calculate the actual analyzed coordinates (same logic as
                // extractROI)
                int actualCenterX = origCenterX;
                int actualCenterY = origCenterY;
                double stabOffsetX, stabOffsetY;
                bool stabActive =
                    m_system.getStabilizationOffset(stabOffsetX, stabOffsetY);
                if (stabActive) {
                ...
                std::string searchStatus =
                    m_focusSearchActive.load() ? " [SEARCHING]" : "";

                std::cout << "[Sensor Fusion] Time: " << timestamp << "ms, ROI("
                          << actualCenterX << "," << actualCenterY << ","
                          << width << "x" << height
                          << "), Sharpness: " << std::fixed
                          << std::setprecision(2) << sharpness << searchStatus
                          << std::endl;
              }
            }
          } catch (const std::exception &e) {
            std::cerr << "[Sensor Fusion] Error processing frame: " << e.what()
                      << std::endl;
          }
        }
      }
...
void SensorFusion::performFocusSearch() {
  ...
  // No depth map available or no valid data in ROI, proceed with hill-climbing
  // search
  std::cout << "[Focus Search] No depth map available, proceeding with "
  ...
  if (desiredLocBestFocus >= FOCUS_MIN && desiredLocBestFocus <= FOCUS_MAX &&
      desiredLocBestFocus != 0) {
    searchStartPosition = desiredLocBestFocus;
    std::cout << "[Focus Search] Starting from current position: "
              << searchStartPosition << std::endl;
  } else {
    searchStartPosition = 400;
    std::cout << "[Focus Search] Using default start position: "
              << searchStartPosition << std::endl;
  }
  ...
  // If starting from default position (400), use longer settle time since this
  // could be a big move
  bool usingDefaultPosition =
      (searchStartPosition == 400 &&
       (desiredLocBestFocus < FOCUS_MIN || desiredLocBestFocus > FOCUS_MAX ||
        desiredLocBestFocus == 0));

  if (usingDefaultPosition) {
    std::cout << "[Sensor Fusion] Using default start position, waiting extra "
                 "time for big move..."
              << std::endl;
  ...
  double initialSharpness =
      getAveragedROISharpness(); // Use averaged measurement
  ...
  if (initialSharpness < 10.0) {
    std::cout << "[Focus Search] Initial sharpness too low, aborting"
              << std::endl;
  ...
  // std::cout << "[Sensor Fusion] Phase 1: Coarse search with step size " <<
  // stepSize << std::endl;
  ...
  std::cout << "[Focus Search] Measuring positions: " << leftPos << " (left), "
            << currentPosition << " (current), " << rightPos << " (right)"
            << std::endl;
  ...
  if (leftPos != currentPosition && steps < MAX_STEPS) {
    ...
    // std::cout << "[Sensor Fusion] Step " << steps << ": position " << leftPos

    if (leftSharpness > bestSharpness) {
      bestPosition = leftPos;
      bestSharpness = leftSharpness;
    }
    std::cout << "[Focus Search] Left sharpness: " << leftSharpness
              << std::endl;
  }
  ...
  if (rightPos != currentPosition && steps < MAX_STEPS) {
    ...
    std::cout << "[Focus Search] Right sharpness: " << rightSharpness
              << std::endl;
  }
  ...
  if (rightSharpness > currentSharpness && rightSharpness > leftSharpness) {
    ...
    std::cout << "[Sensor Fusion] Moving right (positive direction)"
              << std::endl;
  } else if (leftSharpness > currentSharpness &&
             leftSharpness > rightSharpness) {
    direction = -1; // Go left
    currentPosition = leftPos;
    currentSharpness = leftSharpness;
    std::cout << "[Sensor Fusion] Moving left (negative direction)"
              << std::endl;
  } else {
    std::cout
        << "[Focus Search] Current position appears best, starting refinement"
        << std::endl;
  ...
  while (!foundPeakRegion && steps < MAX_STEPS && m_running.load()) {
    ...
    if (nextPosition == currentPosition) {
      std::cout << "[Focus Search] Hit boundary at " << currentPosition
                << std::endl;
      foundPeakRegion = true;
      break;
    }
    ...
    if (nextSharpness > currentSharpness) {
      ...
      std::cout << "[Focus Search] Improved to " << initialSharpness
                << std::endl;
    } else {
      std::cout << "[Focus Search] No improvement, starting refinement"
                << std::endl;
      foundPeakRegion = true;
    }
  }
  ...
  if (foundPeakRegion && steps < MAX_STEPS) {
    // std::cout << "[Sensor Fusion] Phase 2: Fine refinement around position "
    // << bestPosition << std::endl;
    ...
    while (refineStep >= MIN_STEP_SIZE && steps < MAX_STEPS &&
           m_running.load()) {
      // std::cout << "[Sensor Fusion] Refining with step size " << refineStep
      // << std::endl;
      bool improved = false;
      ...
      for (int dir = -1; dir <= 1; dir += 2) // -1 and +1
      {
        ...
        if (testPos != currentPosition) {
          ...
          if (testSharpness > currentSharpness) {
            ...
            // std::cout << "[Sensor Fusion] Improved during refinement!" <<
            // std::endl;
  ...
  // Safety check: if the best position found is significantly worse than
  // initial, return to initial position
  if (bestPosition != initialPosition &&
      bestSharpness < (initialSharpness * 0.8)) {
    finalPosition = initialPosition;
  } else {
    // std::cout << "[Sensor Fusion] Returning to best position found: " <<
    // finalPosition << std::endl;
  }

  // Set final optimal position
  setFocusPositionAndWait(finalPosition);
  double finalSharpness = getAveragedROISharpness(); // Final measurement with
                                                     // averaging for accuracy

  // std::cout << "[Sensor Fusion] Hill-climbing search complete! Final
  // position: " << finalPosition
  //           << ", final sharpness: " << finalSharpness << " (initial: " <<
  //           initialSharpness
  //           << ", best found: " << bestSharpness << ", after " << steps << "
  //           steps)" << std::endl;
  ...
  // Determine if search was successful (any meaningful improvement from
  // initial)
  bool searchSuccessful =
      (bestSharpness > initialSharpness * 1.05); // 5% improvement threshold
  ...
  std::cout << "[Sensor Fusion] Hold focus enabled at position "
            << finalPosition << " (search "
            << (searchSuccessful ? "successful" : "marginal") << ")"
            << std::endl;

  m_focusSearchActive = false;
}

void SensorFusion::setFocusPositionAndWait(int position) {
  setFocusPositionAndWaitLong(position, SETTLE_TIME_MS);
}

void SensorFusion::setFocusPositionAndWaitLong(int position, int settleTimeMs) {
  // Clamp position to valid range
  position = std::max(FOCUS_MIN, std::min(position, FOCUS_MAX));

  // std::cout << "[Sensor Fusion] Setting focus position to " << position
...
double SensorFusion::getCurrentROISharpness() {
  ...
  if (!currentFrame || !currentFrame->data() || currentFrame->size().x <= 0 ||
    ...
    std::cerr
        << "[Sensor Fusion] No valid frame available for sharpness calculation"
        << std::endl;
    return 0.0;
  }
  ...
  try {
    ...
    if (roi.empty()) {
      std::cerr
          << "[Sensor Fusion] Failed to extract ROI for sharpness calculation"
          << std::endl;
      return 0.0;
    }
    ...
    std::cerr << "[Sensor Fusion] Error calculating ROI sharpness: " << e.what()
              << std::endl;
    return 0.0;
  }
}

double SensorFusion::getAveragedROISharpness(int numSamples) {
...
cv::Mat SensorFusion::extractROI(const cv::Mat &frame) {
  if (frame.empty()) {
    return cv::Mat();
  }
  ...
  if (m_useCustomCenter.load()) {
    ...
    // Apply stabilization offset to track the same feature when stabilization
    // is active
    double stabOffsetX, stabOffsetY;
    if (m_system.getStabilizationOffset(stabOffsetX, stabOffsetY)) {
      int originalX = centerX, originalY = centerY;
      // Adjust ROI coordinates OPPOSITE to stabilization offset to track the
      // same feature (if frame rendering moves down, ROI analysis should move
      // up to stay on same feature)
...
double SensorFusion::calculateSharpness(const cv::Mat &region) {
  if (region.empty()) {
    return 0.0;
  }
...
void SensorFusion::performLocalMaxSuppression(
    std::vector<std::vector<std::pair<double, double>>> &depthImage, int width,
    int height) {
  const int REGION_SIZE =
      8; // 8x8 pixel regions - fairly fine grid as requested
  const double FOCUS_TOLERANCE = 10.0; // ±5.0 focus positions as requested
  const double MIN_FOCUS_SEPARATION =
      30.0; // Minimum separation to consider as different peak
  const double SECOND_PEAK_THRESHOLD =
      0.7; // Second peak must be at least 70% of max sharpness
  ...
  for (int regionY = 0; regionY < height; regionY += REGION_SIZE) {
    for (int regionX = 0; regionX < width; regionX += REGION_SIZE) {
      ...
      std::vector<std::tuple<int, int, double, double>>
          validPixels; // x, y, sharpness, focus
      ...
      auto maxIt = std::max_element(
          validPixels.begin(), validPixels.end(),
      ...
      for (int y = regionY; y < endY; y++) {
        for (int x = regionX; x < endX; x++) {
          ...
          if (pixel.first > 0) // Valid pixel
          {
            ...
            bool keepPixel =
                (std::abs(focusPos - primaryFocus) <= FOCUS_TOLERANCE);

            // Also keep pixels that match secondary peak if it exists
            if (secondaryFocus > 0) {
              keepPixel = keepPixel || (std::abs(focusPos - secondaryFocus) <=
                                        FOCUS_TOLERANCE);
            }
  ...
  double suppressionRate =
      (double)pixelsSuppressed / totalValidPixelsBefore * 100.0;

  std::cout << "[Depth Mapping] Local max suppression complete: processed "
            << regionsProcessed << " regions (" << REGION_SIZE << "x"
            << REGION_SIZE << " pixels each)" << std::endl;
...
void SensorFusion::performDepthMapping() {
  std::cout << "\n[Depth Mapping] Starting depth mapping process..."
            << std::endl;
  ...
  // Step 1: Set focus to leftmost position (Hold Focus already enabled in
  // onGetDepthsClicked)
  std::cout << "[Depth Mapping] Setting focus to position " << FOCUS_MIN
            << "..." << std::endl;
  ...
  std::vector<std::vector<std::pair<double, double>>> depthImage(
      reducedHeight,
      std::vector<std::pair<double, double>>(reducedWidth, {-1.0, -1.0}));

  // Store all sharpness values for adaptive thresholding
  std::vector<std::tuple<int, int, double, double>>
      allPixelData; // x, y, sharpness, focus_position
  ...
  allPixelData.reserve(reducedWidth * reducedHeight *
                       ((FOCUS_END - FOCUS_START) / FOCUS_STEP + 1));

  // Process each focus position
  for (int focusPos = FOCUS_START; focusPos <= FOCUS_END;
       focusPos += FOCUS_STEP) {
    if (!m_running.load() || !m_depthMappingActive.load()) {
      std::cout << "[Depth Mapping] Stopped early at focus position "
                << focusPos << std::endl;
      break;
    }
    ...
    cv::Mat originalFrame(originalHeight, originalWidth, CV_8UC1,
                          currentFrame->data());

    // Create a stabilized frame by cropping and padding to maintain original
    // dimensions
    cv::Mat stabilizedFrame =
        cv::Mat::zeros(originalHeight, originalWidth, CV_8UC1);
    ...
    if (hasStabOffset) {
      ...
      // If camera moved right by offsetX, extract from left by offsetX to get
      // same content
    ...
    cv::resize(stabilizedFrame, resizedFrame,
               cv::Size(reducedWidth, reducedHeight));
  ...
  std::cout << "[Depth Mapping] Focus scanning complete! Applying adaptive "
               "thresholding..."
            << std::endl;
  ...
  if (!allPixelData.empty()) {
    ...
    // Find 80th percentile using nth_element (O(n) average case, much faster
    // than sorting)
    const double PERCENTILE = 0.8; // 80th percentile
    size_t percentileIndex =
        static_cast<size_t>(sharpnessValues.size() * PERCENTILE);
    ...
    std::cout << "[Depth Mapping] Adaptive threshold (80th percentile): "
              << adaptiveThreshold << " (from " << allPixelData.size()
              << " candidate pixels)" << std::endl;
    ...
    std::cout << "[Depth Mapping] Kept " << pixelsAboveThreshold
              << " pixels above adaptive threshold ("
              << (100.0 * pixelsAboveThreshold / allPixelData.size())
  ...
  std::cout << "[Depth Mapping] Depth mapping complete! Generated "
            << reducedWidth << "x" << reducedHeight << " depth map"
            << std::endl;
  ...
  try {
    ...
    std::cout << "[Depth Mapping] Creating output directory: " << outputDir
              << std::endl;
    std::filesystem::create_directories(outputDir);

    if (std::filesystem::exists(outputDir)) {
      std::cout
          << "[Depth Mapping] Output directory exists/created successfully"
          << std::endl;
    } else {
      std::cout << "[Depth Mapping] ERROR: Failed to create output directory"
                << std::endl;
      m_depthMappingActive = false;
      return;
    }
    ...
    std::cout << "[Depth Mapping] Attempting to create CSV file: " << filename
              << std::endl;
    ...
    if (csvFile.is_open()) {
      ...
      std::cout << "[Depth Mapping] Successfully exported " << totalValidPixels
                << " depth points to: " << filename << std::endl;
    } else {
      std::cout << "[Depth Mapping] ERROR: Could not create CSV file: "
                << filename << std::endl;
      std::cout << "[Depth Mapping] Check directory permissions and disk space"
                << std::endl;
    }
  } catch (const std::exception &e) {
    std::cout << "[Depth Mapping] ERROR: Exception during CSV export: "
              << e.what() << std::endl;
  }

  std::cout << "[Depth Mapping] Total valid pixels with depth data: "
            << totalValidPixels << std::endl;

  // Transfer depth map data to SDL window for visualization BEFORE moving to
  // system storage
  extern SDLWindow::SDLWin *childwin;
  if (childwin) {
    try {
      SDLWindow::transferDepthMapData(childwin, depthImage, reducedWidth,
                                      reducedHeight);
      std::cout << "[Depth Mapping] Depth map data transferred to SDL window "
                   "for visualization"
                << std::endl;
    } catch (const std::exception &e) {
      std::cout << "[Depth Mapping] ERROR: Exception during SDL transfer: "
                << e.what() << std::endl;
    }
  } else {
    std::cout
        << "[Depth Mapping] SDL window not available for depth map transfer"
        << std::endl;
  }
  ...
  std::cout << "[Depth Mapping] Depth map data stored in system successfully"
            << std::endl;

  m_depthMappingActive = false;
}

cv::Mat SensorFusion::createSmoothedPaddedFrame(const cv::Mat &frame,
  ...
  cv::normalize(blendWeights, blendWeights3, 0.3, 1.0,
                cv::NORM_MINMAX); // Minimum 30% original

  finalFrame = blendWeights3.mul(inpaintedFloat) +
               (1.0 - blendWeights3).mul(blurredFloat);

  // Convert back to original type
  finalFrame.convertTo(result, frame.type());

  // Final step: Apply very light additional smoothing only at the boundary
  // regions
  cv::Mat boundary;
...
cv::Mat SensorFusion::calculateSharpnessImage(const cv::Mat &frame) {
  if (frame.empty()) {
    return cv::Mat();
  }
...
cv::Mat SensorFusion::calculateSharpnessImageTenengrad(const cv::Mat &frame) {
  if (frame.empty()) {
    return cv::Mat();
  }
...
bool SensorFusion::tryDepthMapFocus() {
  ...
  if (validPixelsInROI == 0 || bestFocusPosition < 0) {
    std::cout << "[Sensor Fusion] No valid depth data in ROI region, falling "
                 "back to search"
              << std::endl;
  ...
  std::cout << "[Sensor Fusion] Using depth map: found " << validPixelsInROI
  ...
  std::cout
      << "[Sensor Fusion] Instant focus complete using depth map at position "
