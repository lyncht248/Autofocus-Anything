...
/*
VidFrame::VidFrame(Frame &f) : CVD::Image<VmbUchar_t>(f.buffer,
CVD::ImageRef(f.width, f.height) ), frame(f)
{
}

Frame VidFrame::nullframe = Frame();

VidFrame::VidFrame(CVD::Image<VmbUchar_t> &im) : CVD::VideoFrame<VmbUchar_t>(0,
im), frame(nullframe)
{
}
...
// Class that runs two threads: one which processes frames, and one which
// stabilises them
FrameProcessor::FrameProcessor(System &sys)
    : processed(nullptr), filters(), running(true), _stabNewFrame(false),
      _stabComplete(true), _frameReleased(true), mutex(), stabMutex(),
      stabComplete(), frameReleased(), system(sys), offset(TooN::Zeros),
      currentOff(TooN::Zeros), rasterPos(TooN::Zeros), stabQueue(4),
      released() {
...
FrameProcessor::~FrameProcessor() {
  ...
  if (stabQueue.size() > 0) {
    // for (VidFrame *vframe = stabQueue.pop(); !stabQueue.empty();
    //      vframe = stabQueue.pop())
    //   delete vframe;
    stabQueue.clear();
  }

  if (released.size() > 0) {
    // for (VidFrame *vframe = released.pop(); !released.empty();
    //      vframe = released.pop())
    //   delete vframe;
    released.clear();
  }
  if (bSystemFramesFlag) {
    logger->info("[FrameProcessor::~FrameProcessor] destructor called, "
                 "processorThread and stabThread stopped");
  }
}
...
void FrameProcessor::stabilise() {
  ...
  while (running) {
    // If the queue is backing up, discard older frames and only keep the most
    // recent one or two so we can keep up at higher framerates.
    ...
    while (stabQueue.size() > MAX_ALLOWED_IN_QUEUE) {
      ...
      // No need to delete oldFrame, as it's managed by shared_ptr
    }
    ...
    // Process every frame with the new faster and more accurate phase
    // correlation method
    if (system.usePhaseCorr) {
      ...
      cv::Mat fullFrame(vframe->size().y, vframe->size().x, CV_8UC1,
                        vframe->data());
      ...
      if (frameCount % 30 == 0) {
        ...
        if (bSystemLogFlag) {
          logger->info(
              "[PhaseCorrStabiliser2] Avg time per frame: {:.2f}μs ({:.2f}ms), "
              "Frame size: {}x{}, Raw shift: ({:.3f}, {:.3f})",
              avgTime, avgTime / 1000.0, halfFrame.cols, halfFrame.rows,
              shift.x, shift.y);
        }
        totalTime = 0;
      }
      ...
      stabMutex.lock();
      // Scale shift back up by factor of 2.0 (only for 0.5x downscaling - ROI
      // extraction doesn't affect coordinate scaling) Positive shift means
      // frame moved right/down, so we need to move display left/up to
      // compensate
      TooN::Vector<2> calculatedOffset;
      calculatedOffset[0] =
          shift.x * 2.0f; // 2x scaling to account for 0.5x downscaling
      calculatedOffset[1] =
          shift.y * 2.0f; // 2x scaling to account for 0.5x downscaling

      // Apply the offset directly (no distribution needed with faster
      // stabilizer)
      ...
      if (system.stabiliser.is_valid()) {
        ...
        TooN::Vector<2> calculatedOffset =
            system.stabiliser.stabilise(*vframe, offset);
        ...
        if (oldFrameCount % 30 == 0) {
          ...
          if (bSystemLogFlag) {
            logger->info("[OldStabiliser] Avg time per frame: {:.2f}μs "
                         "({:.2f}ms), Frame size: {}x{}",
                         avgTime, avgTime / 1000.0, vframe->size().x,
                         vframe->size().y);
          }
          oldTotalTime = 0;
        }
        ...
        // Apply the offset directly without distribution (every frame
        // processing)
        currentOff = calculatedOffset;
        offset += currentOff;
        _stabComplete = stabQueue.empty();
...
void FrameProcessor::processFrame() // What to do with each frame received from
                                    // FrameObserver
{
  if (bSystemFramesFlag) {
  ...
  while (running) {
    ...
    if (bSystemFramesFlag) {
      logger->info("[FrameProcessor::processFrame] About to take vidFrame from "
                   "FrameQueue");
    }
    ...
    if (bSystemFramesFlag) {
      logger->info(
          "[FrameProcessor::processFrame] vidFrame taken from FrameQueue");
    }
    ...
    bool recording = system.window.getRecording().getValue() &&
                     !system.window.getPausedRecording().getValue();

    if (bSystemFramesFlag) {
      logger->info("[FrameProcessor::processFrame] recording = {}", recording);
    }
    if (bSystemFramesFlag) {
      logger->info("[FrameProcessor::processFrame] stabilising = {}",
                   stabilising);
    }

    // If recording, sends the frame to the recorder for saving
    if (recording) {
      system.getRecorder().putFrame(
          std::shared_ptr<IVidFrame>(vidFrame)); // putFrame is just frames.push_back(frame); until
                     // frames=recording size
    }
    ...
    if (bSystemFramesFlag) {
      logger->info(
          "[FrameProcessor::processFrame] vidFrame size: {}x{}, about to draw",
          vfDim.x, vfDim.y);
    }
    // Recreate frame if dimensions are different, but raster.w and raster.h
    // should be correct
    if (vfDim.x != childwin->raster.w || vfDim.y != childwin->raster.h) {
      // processed = Cairo::ImageSurface::create(Cairo::FORMAT_RGB24, vfDim.x,
      // vfDim.y);
      SDLWindow::createFrame(childwin, vfDim.x, vfDim.y);
      if (bSystemFramesFlag) {
        logger->info(
            "[FrameProcessor::processFrame] SDLWindow::createFrame called");
      }
    }
    ...
    // If stabilising, render frame using stabilised offset, otherwise just
    // render normally
    if (stabilising) {
      ...
      rasterPos[1] -=
          currentOff[1]; // y offset (no correction needed with new stabilizer)
      ...
      if (bSystemFramesFlag) {
        logger->info(
            "[FrameProcessor::processFrame] SDLWindow::setRaster called");
      }
    }
    system.signalNewFrame().emit();
    if (bSystemFramesFlag) {
      logger->info(
          "[FrameProcessor::processFrame] system.signalNewFrame called");
    }

    // Now that the current vframe is processed, release it
    if (bSystemFramesFlag) {
      logger->info("[FrameProcessor::processFrame] About to release vidFrame, "
                   "locking mutex");
    }
    ThreadStopper::lock(mutex);
    {
      // When a frame is drawn, it is signalled for release (see code below)
      // window.signalFrameDrawn().connect(sigc::mem_fun(*this,
      // &System::releaseFrame) );
      ...
      // If not stabilising, add frame to released queue. If you are
      // stabilising, it should already have been added if (!stabilising)
      // {
      released.push(vidFrame);
      if (bSystemFramesFlag) {
        logger->info(
            "[FrameProcessor::processFrame] vidFrame pushed to released queue");
      }
      // }

      // //// PREVIOUS ////
      // // If live view is ON and not recording, delete every frame in the
      // released queue if (system.window.getLiveView().getValue() &&
      // !recording)
      // {
      // 	// Deletes every frame in the released queue... same as
      // released.clear()? 	for (VidFrame *vframe = released.pop();
      // !released.empty(); vframe = released.pop() ) 		delete vframe;
      // vidFrame = nullptr;
      // }

      // // Otherwise, delete the frame! (if live view is off (ie. on a
      // recording) or live view is on and currently taking a recording), then
      // clear the released queue else
      // {
      // 	released.clear();
      // 	if(bSystemLogFlag) {logger->info("[FrameProcessor::processFrame]
      // released queue cleared, but NOT deleted!!!");}
      // }

      //// NEW ////
      if (recording || !system.window.getLiveView().getValue()) {
        // If recording or viewing a recording, then don't delete the frame data
        // (which is pointed to by recorder::frames), but clear the released
        // queue of pointers
        released.clear();

        if (bSystemLogFlag) {
          logger->info("[FrameProcessor::processFrame] released queue cleared, "
                       "but data not deleted!!!");
        }
      } else {
        ...
        // 	delete vframe;
        // }
        // if(bSystemFramesFlag) logger->info("[FrameProcessor::processFame]
        // released queue cleared");
        //     // Not recording, so can delete the released frames.
        // for (VidFrame *vframe = released.pop(); !released.empty(); vframe =
        // released.pop() ) 	delete vframe; vidFrame = nullptr;
        // released.clear();

        // This code fixes a problem where the frame memory would leak (steadily
        // increasing RAM use) even when not recording
        //  using >=5 because often the frame data is still being used to draw
        //  the frame when this code is called
        if (released.size() >= 5) {
          std::shared_ptr<IVidFrame> frame_to_delete =
              released.pop();     // remove the frame from the queue
          // No need to delete frame_to_delete, as it's managed by shared_ptr
          if (bSystemFramesFlag) {
            logger->info("[FrameProcessor::ProcessFrame] Deleted a frame from "
                         "the released queue.");
          }
        }
      }
    ...
    if (bSystemFramesFlag) {
      logger->info("[FrameProcessor::processFrame] stabQueue is length: {}",
                   stabQueue.size());
    }
    ...
    if (bSystemFramesFlag) {
      logger->info(
          "[FrameProcessor::processFrame] released queue is length: {}",
          released.size());
    }

    if (bSystemFramesFlag) {
      logger->info("[FrameProcessor::processFrame] frameQueue is length: {}",
                   system.getFrameQueue().size());
    }
    ...
    // Imaging camera monitoring will be started only when user double-clicks to
    // set ROI

    // Check for double-click events from SDL window
    if (childwin && vidFrame) {
      pthread_mutex_lock(&childwin->mutex);
      if (childwin->lcmd[12].d_bool) { // Check if we have a new ROI center
        int imageX =
            childwin->lcmd[10]
                .d_int; // Already converted to image coordinates by SDL child
        ...
        roiSizePixels =
            std::max(20, std::min(roiSizePixels,
                                  500)); // Clamp between 20 and 500 pixels
        ...
        if (bSystemLogFlag) {
          logger->info("[FrameProcessor::processFrame] Starting imaging camera "
...
void FrameProcessor::clearQueues() {
  ...
  // Log the sizes of the queues to verify they are cleared
  if (bSystemMemoryLeakFlag) {
      logger->info("[FrameProcessor::clearQueues] frameQueue size after clearing: {}", system.getFrameQueue().size());
      logger->info("[FrameProcessor::clearQueues] stabQueue size after clearing: {}", stabQueue.size());
      logger->info("[FrameProcessor::clearQueues] released queue size after clearing: {}", released.size());
  }
...
// This class is used to receive frames from the camera, but passes frames
// immediately to FrameProcessor
class FrameObserver : public Glib::Object,
                      virtual public Vimba::IFrameObserver {
public:
  FrameObserver(Vimba::CameraPtr pCamera, TSQueue<std::shared_ptr<IVidFrame>> &frameQueue)
      : IFrameObserver(pCamera), Object(), sigReceived(),
  ...
  ~FrameObserver() {
    if (sysFrames.size() > 0) {
      for (std::shared_ptr<IVidFrame> vframe = sysFrames.pop(); !sysFrames.empty();
           vframe = sysFrames.pop())
        // No need to delete vframe, as it's managed by shared_ptr
      sysFrames.clear();
    }
  }
  ...
  void FrameReceived(const Vimba::FramePtr pFrame) {
    ...
    if (VmbErrorSuccess == pFrame->GetReceiveStatus(eReceiveStatus)) {
      if (VmbFrameStatusComplete == eReceiveStatus) {
        ...
        pFrame->GetImageSize(
            sz); // TODO: These should be called once, not every frame
        pFrame->GetWidth(width);
        pFrame->GetHeight(height);

        if (bSystemFramesFlag) {
          logger->info("[FrameObserver::FrameReceived] Frame size: {}x{}",
                       width, height);
        }
        // IVidFrame *sysFrame = new IVidFrame(
        //     CVD::ImageRef(width, height)); // Each of these must get deleted
        //                                    // when processed by FrameProcessor
        ...
        if (bSystemFramesFlag) {
          logger->info("[FrameObserver::FrameReceived] sysFrame pushed to "
                       "sysFrames queue");
        }

        // Here is where we check the queue size and drop a frame if needed.
        // TODO: just change in frameQueue in System::System
        if (sysFrames.size() >= 15) {
          ...
          if (bSystemFramesFlag) {
            logger->info(
                "[FrameObserver::FrameReceived] Popped a frame (should "
                "auto-delete) from the sysFrames queue as it was too long.");
          }
        }

        // Quickly recieves 15 frames which are then emptied by FrameProcessor
        // quickly
        if (bSystemFramesFlag) {
          logger->info(
              "[FrameObserver::FrameReceived] sysFrames queue is length: {}",
              sysFrames.size());
        }
  ...
  SignalReceived signalReceived() { return sigReceived; }
...
System::Private::Private() : observer(), frames(3) {}

System::System(int argc, char **argv)
    : window(), vsys(Vimba::VimbaSystem::GetInstance()), cam(), size(), im(),
      priv(new Private()), sigNewFrame(hvigtk_threadcontext()), frameQueue(100),
      frameProcessor(*this), thread(),
      recorder(thread.createObject<Recorder, System &>(*this)),
      sRecorderOperationComplete(), stabiliser(), madeMap(false), AF(),
  ...
  sRecorderOperationComplete.connect(
      sigc::mem_fun(*this, &System::onRecorderOperationComplete));

  window.signal_map_event().connect([this](GdkEventAny * /*event*/) -> bool {
    window.move(gtkAppLocationX,
                gtkAppLocationY); // Replace with your desired coordinates.
    return false;                 // Continue propagation.
  });

  window.signalFrameDrawn().connect(
      sigc::mem_fun(*this, &System::releaseFrame));
  window.signalFeatureUpdated().connect(
      sigc::mem_fun(*this, &System::onWindowFeatureUpdated));
  // window.signalThresholdChanged().connect(sigc::mem_fun(*this,
  // &System::onWindowThresholdChanged) );
  // window.signalScaleChanged().connect(sigc::mem_fun(*this,
  // &System::onWindowScaleChanged) );
  window.signalBestFocusChanged().connect(
      sigc::mem_fun(*this, &System::onWindowBestFocusChanged));
  window.signalPauseClicked().connect(
      sigc::mem_fun(*this, &System::onWindowPauseClicked));
  sigNewFrame.connect(sigc::mem_fun(*this, &System::renderFrame));

  window.signalEnterClicked().connect(
      sigc::mem_fun(*this, &System::onWindowEnterClicked));

  // CONDITIONS

  window.getLiveView().signalToggled().connect(
      sigc::mem_fun(*this, &System::whenLiveViewToggled));

  // window.getMakeMapActive().signalToggled().connect(sigc::mem_fun(*this,
  // &System::whenMakeMapToggled) );
  window.getStabiliseActive().signalToggled().connect(
      sigc::mem_fun(*this, &System::whenStabiliseToggled));
  // window.getShowMapActive().signalToggled().connect(sigc::mem_fun(*this,
  // &System::whenShowMapToggled) );
  ...
  window.signalGetDepthsClicked().connect(
      sigc::mem_fun(*this, &System::onGetDepthsClicked));
  window.getGetDepthsActive().signalToggled().connect(
      sigc::mem_fun(*this, &System::whenGetDepthsToggled));

  window.getHoldFocusActive().signalToggled().connect(
      sigc::mem_fun(*this, &System::whenHoldFocusToggled));
  window.getViewDepthsActive().signalToggled().connect(
      sigc::mem_fun(*this, &System::whenViewDepthsToggled));
  window.get3DStabActive().signalToggled().connect(
      sigc::mem_fun(*this, &System::when3DStabToggled));
  window.get2DStabActive().signalToggled().connect(
      sigc::mem_fun(*this, &System::when2DStabToggled));

  window.getLoading().signalToggled().connect(
      sigc::mem_fun(*this, &System::whenLoadingToggled));
  window.getSaving().signalToggled().connect(
      sigc::mem_fun(*this, &System::whenSavingToggled));
  window.getPlayingBuffer().signalToggled().connect(
      sigc::mem_fun(*this, &System::whenPlayingToggled));
  window.getSeeking().signalToggled().connect(
      sigc::mem_fun(*this, &System::whenSeekingToggled));
  window.getRecording().signalToggled().connect(
      sigc::mem_fun(*this, &System::whenRecordingToggled));

  // Signals that the window has been closed, and other threads must close
  window.signal_delete_event().connect(
      sigc::mem_fun(*this, &System::onCloseClicked));
  ...
  NotificationCenter::instance().registerListener(
      "outOfBoundsError", [&]() { window.showOutOfBoundsWarning(); });

  // Add a listener for when lens moves back in bounds
  NotificationCenter::instance().registerListener(
      "lensInBounds", [&]() { window.hideOutOfBoundsWarning(); });
  ...
  window.signalPGainChanged().connect(
      sigc::mem_fun(*this, &System::onWindowPGainChanged));

  window.signalSettingsChanged().connect(
      sigc::mem_fun(*this, &System::onWindowSettingsChanged));

  sigSharpnessUpdated.connect(
  ...
  // Imaging camera monitoring will be started when user double-clicks to set
  // ROI
}
...
bool System::startStreaming() {
  ...
  if (cam != nullptr) {
    ...
    if (cam->Open(VmbAccessModeFull) == VmbErrorSuccess) {
      ...
      window.updateCameraValues(getFeature<double>("Gain"),
                                getFeature<double>("ExposureTime"),
                                getFeature<double>("Gamma"));
      setFeature("PixelFormat", VmbPixelFormatMono8);
      setFeature("AcquisitionFrameRateEnable", true);
      FPSsuccess = setFeature<double>("AcquisitionFrameRate",
                                      window.getFrameRateEntryBox());
      ...
      priv->observer.reset(new FrameObserver(
          cam, frameQueue)); // reset means its a smart pointer and will delete
                             // itself when it goes out of scope

      for (Vimba::FramePtrVector::iterator iter = priv->frames.begin();
           iter != priv->frames.end(); ++iter) // iter is the frame number
      {
        (*iter).reset(
            new Vimba::Frame(nPLS)); // create a frame of the correct size
        if (bSystemFramesFlag) {
          logger->info("[System::startStreaming] Frame {} created",
                       iter - priv->frames.begin());
        }
        (*iter)->RegisterObserver(priv->observer); // register the observer
        if (bSystemFramesFlag) {
          logger->info("[System::startStreaming] Frame {} registered",
                       iter - priv->frames.begin());
        }
        cam->AnnounceFrame(*iter); // announce the frame to the camera
        if (bSystemFramesFlag) {
          logger->info("[System::startStreaming] Frame {} announced",
                       iter - priv->frames.begin());
        }
      }
      ...
      if (cam->StartCapture() == VmbErrorSuccess) {
        ...
        for (Vimba::FramePtrVector::iterator iter = priv->frames.begin();
          ...
          cam->QueueFrame(*iter);
          if (bSystemFramesFlag) {
            logger->info("[System::startStreaming] Frame {} queued",
                         iter - priv->frames.begin());
          }
        }

        if (cam->GetFeatureByName("AcquisitionStart", pFeature) ==
                VmbErrorSuccess &&
            pFeature->RunCommand() == VmbErrorSuccess) {
          if (bSystemLogFlag) {
      ...
      fail:;
        ...
        if (bSystemLogFlag) {
          logger->info(
              "[System::startStreaming] Failed to start frame capture");
...
void System::stopStreaming() {
  if (cam != nullptr) {
    ...
    if (cam->GetFeatureByName("AcquisitionStop", pFeature) == VmbErrorSuccess &&
        pFeature->RunCommand() == VmbErrorSuccess) {
      if (bSystemLogFlag) {
...
TSQueue<std::shared_ptr<IVidFrame>> &System::getFrameQueue() { return frameQueue; }

Glib::Dispatcher &System::signalNewFrame() { return sigNewFrame; }

std::shared_ptr<IVidFrame> System::getFrame() { return frameProcessor.vidFrame; }

void System::renderFrame() { window.renderFrame(frameProcessor.vidFrame); }
...
void System::releaseFrame() {
  ...
  // !window.get2DStabActive().getValue()) { //If either 3D or 2D stab is
  // active, don't show map 		window.setShowingMap(true);
  // 	}
  // 	madeMap = true;
  // }
...
//  	if (makingMap)
//  	{
//  		if(bSystemLogFlag) {logger->info("[System::whenMakeMapToggled]
//  Making map toggled on");} 		frameProcessor.resetRaster(); if
//  (!window.getLiveView().getValue() )
//  		{
//  			VidFrame *vframe =
//  recorder->getFrame(window.getFrameSliderValue() ); 			if
//  (vframe)
//  			{
//  				stabiliser.make_map(*vframe,
//  DEFAULT_NUM_TRACKERS);
//  if(!window.get3DStabActive().getValue() &&
//  !window.get2DStabActive().getValue()) {  //If either 3D or 2D stab is
//  active, don't show map
...
// 	if (showingMap)
// 	{
// 		if(bSystemLogFlag) {logger->info("[System::whenShowMapToggled]
// Show map toggled on");} 		stabiliser.predraw();
...
// 	{
// 		if(bSystemLogFlag) {logger->info("[System::whenShowMapToggled]
// Show map toggled off");} 		frameProcessor.filters.erase("map");
// 		SDLWindow::setShowingMap(childwin, false);
// 	}

// 	/*
// 	if (!window.getLiveView().getValue() &&
// !window.getPlayingBuffer().getValue() )
// 	{
// 		VidFrame *vframe =
// recorder->getFrame(window.getFrameSliderValue()
// ); 		if (vframe)
// 		{
// 			frameQueue.clear();
...
void System::onResetClicked() {
  if (bSystemLogFlag) {
    logger->info("[System::onResetClicked] Reset All button clicked - "
                 "performing comprehensive system reset");
  }
  ...
  if (bSystemLogFlag) {
    logger->info("[System::onResetClicked] Reset All complete - all systems "
                 "cleared and reset to defaults");
  }
}
...
void System::whenPlayingToggled(bool playing) {
  if (playing) {
    ...
    if (!recorder->isBuffering()) {
      if (bSystemLogFlag) {
        logger->info("[System::whenPlayingToggled] Start playing from frame: "
                     "{} at rate: {}",
                     start, frameRate);
      }
    ...
    if (bSystemLogFlag) {
      logger->info("[System::whenPlayingToggled] Playing toggled off (probably "
                   "by 'Pause')");
    }
    // TODO: Need to delete frames in frameQueue? Or manage in recorder?
    recorder->stopBuffering();
    // frameQueue.clear(); //This queue is the frames waiting to be rendered,
    // NOT those in the recorder
  }
}
...
void System::whenSeekingToggled(bool seeking) {
  if (seeking) {
    ...
    if (window.getPlayingBuffer().getValue()) {
      ...
      if (out)
        frameQueue.push(
            out); // frameQueue is the same as sysQueue in FrameObserver
...
void System::onRecorderOperationComplete(RecOpRes res) {
  ...
  case Recorder::Operation::RECOP_ADDFRAME: {
    if (window.getLoading().getValue())
      ...
      window.displayMessageFPS(std::string("Recorded frame: ") +
                               std::to_string(recorder->countFrames()) +
                               std::string(" of ") +
...
void System::onWindowThresholdChanged(double val) {
  if (bSystemLogFlag) {
    logger->info("[System::onWindowThresholdChanged] Threshold changed to: {} "
                 "recalculating map",
                 val);
  }
  frameProcessor.stabMutex.lock();
...
void System::onWindowScaleChanged(double val) {
  if (bSystemLogFlag) {
    logger->info(
        "[System::onWindowScaleChanged] Scale changed to: {} recalculating map",
        val);
  }
...
void System::onWindowBestFocusChanged(double val) {
  ...
  if (bSystemLogFlag) {
    ...
    auto timestamp_ms = std::chrono::duration_cast<std::chrono::microseconds>(
                            now.time_since_epoch())
                            .count() /
                        1000.0;
    logger->info("[System::onWindowBestFocusChanged] Best focus location "
                 "changed to: {}, updating desiredLocBestFocus, "
                 "Timestamp: {:.3f}ms",
                 val, timestamp_ms);
  }

  int val2 = round(val);
  // TODO: This should be in a mutex, and done much better. but it's not
  // critical
  ...
  if (childwin && !sensorFusionUpdatingFocus) {
    ...
    // If there was a completed ROI search and user manually changed focus,
    // clear it
    if (wasSearchComplete && hasActiveROI) {
      if (bSystemLogFlag) {
        logger->info("[System::onWindowBestFocusChanged] User manually changed "
                     "focus after ROI search - clearing ROI tracking");
      }
...
void System::onWindowEnterClicked() {
  ...
  if (0.0 < newFrameRate && newFrameRate < 144.0) {
    ...
    if (recorder->isBuffering()) // if Buffering, must restart buffering at new
                                 // frame rate
    {
      recorder->stopBuffering();
      usleep(100000); // sleep for 0.1s
      recorder->signalBuffer().emit(
          std::make_pair(window.getFrameSliderValue(), newFrameRate));
      window.setPlayingBuffer(true);
    }
    // If neither buffering nor viewing live (i.e. paused recording), do nothing
    // as new frame rate  will be picked up when 'play' clicked
...
System::~System() {
  ...
  if (frameQueue.size() > 0) {
    // for (auto vframe = frameQueue.pop(); !frameQueue.empty();
    //      vframe = frameQueue.pop())
    //   delete vframe;
    frameQueue.clear();
  }
...
MainWindow &System::getWindow() { return window; }

Recorder &System::getRecorder() { return *recorder; }
...
void System::handleDisconnection() {
  ...
  if (tiltedCamDisconnected && sensorFusionDisconnected && lensDisconnected) {
    ...
    std::cout << "Tilted Camera and Imaging Camera are Disconnected!"
              << std::endl;
    window.displayMessageError(
        "Imaging Camera and Tilted Camera are Disconnected");
...
void System::onWindowHomePositionChanged(double val) {
  if (bSystemLogFlag) {
    logger->info("[System::onWindowHomePositionChanged] Lens home position "
                 "changed to: {}mm",
                 val);
  }
...
void System::onGetDepthsClicked() {
  ...
  if (gettingDepths) {
    ...
    if (!window.getLiveView().getValue()) {
      window.displayMessageError(
          "Depth mapping only available in live view mode");
    ...
    if (bSystemLogFlag) {
      logger->info("[System::onGetDepthsClicked] Clearing stabilization and "
                   "resetting offsets");
    }
    ...
    if (bSystemLogFlag) {
      logger->info(
          "[System::onGetDepthsClicked] Clearing existing depth map data");
    }
...
void System::whenGetDepthsToggled(bool gettingDepths) {
  ...
  if (bSystemLogFlag) {
    logger->info(
        "[System::whenGetDepthsToggled] Get Depths toggle state changed to: {}",
        gettingDepths);
  }
}
...
void System::whenViewDepthsToggled(bool viewingDepths) {
  if (bSystemLogFlag) {
    logger->info("[System::whenViewDepthsToggled] View Depths toggled: {}",
                 viewingDepths);
  }
...
void System::updateROICenter(int x, int y) {
  ...
  if (!window.getLiveView().getValue()) {
    if (bSystemLogFlag) {
      logger->info("[System::updateROICenter] ROI update ignored - not in live "
                   "view mode");
    }
    return;
  }
...
void System::clearROIDisplay() {
  ...
  if (getSensorFusion()) {
    ...
    if (bSystemLogFlag) {
      logger->info("[System::clearROIDisplay] Stopped imaging camera "
                   "monitoring and focus search threads");
    }
  }
  ...
  if (childwin) {
    ...
    if (bSystemLogFlag) {
      logger->info(
          "[System::clearROIDisplay] ROI display cleared from SDL window");
    }
  }
}
...
bool System::getStabilizationOffset(double &offsetX, double &offsetY) {
  ...
  if (stabilising) {
    // Use try_lock to avoid deadlock if stabMutex is already held by current
    // thread
    if (frameProcessor.stabMutex.trylock()) {
      ...
      // If we can't get the lock, use the last known values from SDL shared
      // memory This avoids deadlock when called from within stabilization
      // thread
      if (childwin) {
...
void System::createStabiliserMapWithDefaults(const IVidFrame &frame) {
  if (bSystemLogFlag) {
    logger->info("[System::createStabiliserMapWithDefaults] Creating "
                 "stabilizer map with automatic parameters");
  }
  ...
  double threshold =
      calculateThresholdForPercentage(stab_image, defaultScale, 0.10);

  if (bSystemLogFlag) {
    logger->info("[System::createStabiliserMapWithDefaults] Using scale: {}, "
                 "threshold: {}",
  ...
  if (stabiliser.is_valid()) {
    ...
    if (bSystemLogFlag) {
      logger->info("[System::createStabiliserMapWithDefaults] Map created and "
                   "displayed successfully");
    }
  } else {
    if (bSystemLogFlag) {
      logger->warn("[System::createStabiliserMapWithDefaults] Failed to create "
                   "valid stabilizer map");
    }
  }
}

double
System::calculateThresholdForPercentage(const CVD::Image<unsigned char> &image,
                                        double vesselSize,
  ...
  // Apply same processing as stabilizer: Gaussian blur then eigenvalue
  // calculation
  ...
  do {
    ...
    double b = 0.25 * (blurred[scan - dx - dy] + blurred[scan + dx + dy] -
                       blurred[scan - dx + dy] - blurred[scan + dx - dy]);
  ...
  size_t targetIndex =
      static_cast<size_t>(eigenvalues.size() * targetPercentage);
  ...
  if (bSystemLogFlag) {
    logger->info("[System::calculateThresholdForPercentage] Calculated "
                 "threshold {} for {}% pixel selection from {} eigenvalues",
                 calculatedThreshold, targetPercentage * 100,
                 eigenvalues.size());
  }
